...
function generateUniqueKey(file) {
    return Date.now();
}
...
http.listen(PORT, function () {
    ...
    mongoClient.connect(connectionString, {
        ...
        app.get("/viewDocumentsForm", function (request, result) {
            result.render("viewDocumentsForm", {
                "request": request
            });
        });

        app.get("/viewDocuments", async function (request, result) {
            const uniqueId = request.query.uniqueId;
        
            try {
                const user = await database.collection("users").findOne({ "uniqueId": uniqueId });
        
                if (user) {
                    result.render("viewDocuments", {
                        "request": request,
        ...
        app.post("/UploadFile", async function (request, result) {
            if (request.session.user) {
                const uniqueId = request.fields.uniqueId;
                ...
                if (!user) {
                    request.session.status = "error";
                    request.session.message = "User with the provided Unique ID does not exist.";
                ...
                if (request.files.file.size > 0) {
                    var uploadedObj = {
                        ...
                        "filePath": "",
                        "createdAt": new Date().getTime(),
                        "uniqueKey": generateUniqueKey(request.files.file)
                    };
                    ...
                    fileSystem.readFile(request.files.file.path, async function (err, data) {
                        ...
                        fileSystem.writeFile(filePath, data, async function (err) {
                            ...
                            // Update user's uploaded files array
                            await database.collection("users").updateOne({
            ...
            if (user == null) {
                bcrypt.hash(password, 10, async function (error, hash) {
                    await database.collection("users").insertOne({
                        ...
                        request.status = "success";
                        request.message = `Signed up successfully. Your unique ID is ${uniqueId}. You can login now.`;
